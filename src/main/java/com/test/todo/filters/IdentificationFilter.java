...
public class IdentificationFilter extends HttpFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Filter to check if the user exists already in the database

        HttpServletRequest req = (HttpServletRequest) request;
        HttpServletResponse res = (HttpServletResponse) response;
        boolean flag = false;

        // Checking if the request is a POST and targets specific URLs
        if (req.getMethod().equalsIgnoreCase("POST") &&
                (req.getRequestURI().startsWith("/TodoApp/RegisterUser") ||
            ...
            // If trying to sign up with existing credentials, redirect to the login page
            ...
            // If trying to login without registering first, redirect to the registration page
        ...
        if (!flag) { // If no problem detected, continue with the filter chain
